    ...
    children=[
        ...
        html.Div(style={"display": "flex"}, children=[
            ...
            html.Div(style={"width": "25%", "padding": "20px"}, children=[
                ...
                dcc.Dropdown(
                    options=[
                        {"label": pd.Period(m).strftime("%B %Y"), "value": m}
...
def update(month_sel, end_date, teams, agents, id_sel, affiliates, countries):
    ...
    if month_sel:
        ...
        rtn = dff[
            ...
            (dff["id"].isin(ftd_base["id"])) &
            (dff["date"] > ftd_base.set_index("id").loc[dff["id"]]["date"].values) &
            (dff["date"] <= end)
        ]

        std_df = rtn.sort_values("date").groupby("id", as_index=False).first()
